...
class LocoNew(FSMState):
    def __init__(self, state_cmd: StateAndCmd, policy_output: PolicyOutput):
        ...
        self.name = FSMStateName.LOCO_NEW
        self.name_str = "loco_new_mode"

        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, "config", "LocoNew.yaml")
        with open(config_path, "r") as f:
            ...
            with torch.inference_mode():
                ...
                assert probe.shape[-1] == self.num_actions, (
                    f"LocoNew model output dim {probe.shape} does not match num_actions={self.num_actions}"
                )

            # Spot-check Isaac 29-DoF ordering vs motor ordering.
            ...
            print("LocoNew policy initializing ...")
