...
Test every change in MuJoCo before touching a real robot. Only `LocoMode`, `FixedPose`, `PassiveMode`, and `Dance` are considered stable on hardware; other skills are sim-only or experimental. `LocoNew` (99-dim obs incl. `base_lin_vel`, bound to `x+l1` / `y+r1` for ONNX) is sim2sim-only — `deploy_real.py` passes a zeroed `base_lin_vel` since the IMU doesn't provide body-frame linear velocity directly. `BoxTransportVelocity` (96-dim single-frame obs, ONNX, bound to `b+r1`) is sim2sim-only and has no hardware binding. `DualAgentVelocity` (dual-actor ONNX: upper 480-dim 5-frame history + lower 99-dim single-frame, output already in MuJoCo motor order, bound to `a+l1`) is also sim2sim-only — the lower actor consumes body-frame `base_lin_vel` from `mj_objectVelocity`, which the real-robot IMU doesn't provide. `DualAgentTracking` (dual-actor ONNX: upper 480-dim 5-frame history + lower 121-dim single-frame, bound to `c+l1`) is sim2sim-only motion-tracking — it loads a preprocessed motion npz from `policy/dual_agent_tracking/motion/walk_tracking_ref.npz` at `enter()`, advances one frame per 50 Hz control tick, and consumes world-frame `torso_link` pose (injected into `state_cmd.anchor_pos_w`/`anchor_quat_w` by the deploy loops from `d.xpos`/`d.xquat`) to build `motion_anchor_pos_b`/`ori_b`. It has `needs_transport_box = False` (training scene has no box). Both box-carrying policies declare `needs_transport_box = True` on their `FSMState` subclass; the deploy loops read that flag (not the policy name) to decide whether to teleport the shared `transport_box` body into the grasp region. In MuJoCo, `scene.xml` carries a `transport_box` body parked at `(100, 100, 0.15)`; on `needs_transport_box` entry the deploy loops teleport it to a pelvis-frame offset and hard-pin qpos/qvel for 1s (rope-hold) before releasing to physics. Other policies must keep `d.qpos[7:7+num_joints]` / `d.qvel[6:6+num_joints]` slices bounded now that `d.qpos/qvel` extend past the robot.
