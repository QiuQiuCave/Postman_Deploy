...
class BoxHandoffStand(FSMState):
    """
    First no-tracking box-handoff policy.

    The actor observation matches BoxTransportVelocity's 96-dim single-frame
    contract, but the velocity command is always zero and no MuJoCo transport
    box is spawned. Until a trained ONNX is installed, the state falls back to
    holding the configured handoff default pose.
    """
    ...
    def __init__(self, state_cmd: StateAndCmd, policy_output: PolicyOutput):
        ...
        self.name = FSMStateName.BOX_HANDOFF_STAND
        self.name_str = "box_handoff_stand_mode"

        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, "config", "BoxHandoffStand.yaml")
        ...
        print("BoxHandoffStand policy initializing (lazy ONNX load) ...")
    ...
    def _load_policy_if_available(self):
        ...
        if not os.path.exists(self.policy_path):
            self.use_policy = False
            print(f"BoxHandoffStand: missing ONNX at {self.policy_path}; holding default pose.")
            return
        ...
        self.use_policy = True
        print("BoxHandoffStand: ONNX loaded, starting learned policy after ramp.")
    ...
    def enter(self):
        ...
        print(f"BoxHandoffStand: ramping to handoff pose over {self.ramp_time:.2f}s "
    ...
    def run(self):
        if self.ramping:
            ...
            if alpha >= 1.0:
                self.ramping = False
                if self.use_policy:
                    print("BoxHandoffStand: ramp complete, starting policy inference.")
                else:
                    print("BoxHandoffStand: ramp complete, holding default pose.")
            return
    ...
    def checkChange(self):
        ...
        elif self.state_cmd.skill_cmd == FSMCommand.BOX_HOLD_STAND:
            self.state_cmd.skill_cmd = FSMCommand.INVALID
