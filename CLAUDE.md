...
Test every change in MuJoCo before touching a real robot. Only `LocoMode`, `FixedPose`, and `PassiveMode` are considered stable on hardware; `DualAgentTracking` (post-2026-04-21 slim retrain) is real-deployable in principle but only with the box-handoff and verification ladder in `refer/DualAgentTracking-Sim2Real-Guide.md`. Other skills are sim-only or experimental. Joystick/keyboard bindings are grouped so **R1-combos** cover original locomotion plus staged box handoff and **L1-combos** cover the motion-tracking family. Current R1 layout: `a+r1` → `LocoMode`, `b+r1` → `BoxHandoffStand` (no-box, no-tracking open-hands handoff policy, 96-dim single-frame ONNX), `x+r1` → `BoxHoldStand` (box-present no-tracking clamp policy, 96-dim single-frame ONNX; MuJoCo spawns the shared box), `y+r1` unbound. Current L1 layout: `a+l1` → `DualAgentTracking` (walk demo, dual-actor ONNX: upper 96-dim single-frame + lower 109-dim single-frame; iter-15000 of `2026-04-21_20-01-44_joint_train`) and `b+l1` → `DualAgentRunTracking` (run demo, sim2sim + deploy_real entry wired, upper/lower 96/109 slim obs; iter-13000 of `2026-05-05_02-02-39_run1_subject2_resume2500`; hardware still needs the validation ladder) — each loads a preprocessed motion npz at `enter()` and advances one frame per 50 Hz control tick. The earlier 480/121 variant consumed world-frame `torso_link` pose and body-frame `base_lin_vel`; the slim retrain moved those three obs terms (`motion_anchor_pos_b`, `motion_anchor_ori_b`, `base_lin_vel`) to the critic, so the actor now needs only IMU + encoders + motion phase — that's why walk tracking can ride the L1 path *and* the real-deploy path. The deploy loops still inject `anchor_pos_w`/`anchor_quat_w` from `d.xpos`/`d.xquat` and `base_lin_vel` from `mj_objectVelocity` into `state_cmd` for forward-compat, but the tracking policies ignore them. `x+l1`, `y+l1` are reserved for future tracking demos; each new tracking skill pairs its own ONNX + its own motion npz (cannot just swap motion_file on a shared ONNX). `LocoNew` / `LocoNewOnnx` / `Dance` / old box-transport classes still live in `FSM.__init__` and transition logic, but have no active R1 keybinding — their code is kept for programmatic re-binding or debugging. `DualAgentTracking`, `DualAgentRunTracking`, `DualAgentBoxTransVel`, `BoxTransportVelocity`, and `BoxHoldStand` declare `needs_transport_box = True` on their `FSMState` subclass; `BoxHandoffStand` deliberately does not. The deploy loops read that flag (not the policy name) to decide whether to teleport the shared `transport_box` body into the grasp region. In MuJoCo, `scene.xml` carries a `transport_box` body parked at `(100, 100, 0.15)`; on `needs_transport_box` entry the deploy loops teleport it to a pelvis-frame offset and hard-pin qpos/qvel for 1s (rope-hold) before releasing to physics. On real hardware the box must be handed to the robot manually after the ramp completes — see the sim2real guide. Other policies must keep `d.qpos[7:7+num_joints]` / `d.qvel[6:6+num_joints]` slices bounded now that `d.qpos/qvel` extend past the robot.
